...
const listItemStyle = { marginBottom: '0.6rem', paddingLeft: '1.5rem', position: 'relative', color: 'var(--secondary)', fontSize: '0.95rem' };
const listIconStyle = { position: 'absolute', left: 0, color: 'var(--accent)', fontWeight: 'bold' };
  ...
  return (
    <>
      <PageHero
        badge="What We Do"
        title="Services"
        subtitle="Specialist support across planning, project controls, delay analysis and commercial administration."
        extra={
          <>
            <div style={{ position: 'absolute', top: '-80px', right: '-80px', width: '400px', height: '400px', borderRadius: '50%', border: '1px solid rgba(217,154,43,0.15)', animation: 'rotateSlow 30s linear infinite', pointerEvents: 'none' }} />
            <div style={{ position: 'absolute', top: '-30px', right: '-30px', width: '300px', height: '300px', borderRadius: '50%', border: '1px solid rgba(217,154,43,0.1)', animation: 'rotateSlow 20s linear infinite reverse', pointerEvents: 'none' }} />
          </>
        }
      >
        {/* Stats */}
        <div style={{ display: 'flex', gap: '3rem', marginTop: '3rem', flexWrap: 'wrap' }}>
          {[{ value: '4', label: 'Core Services' }, { value: '10+', label: 'Years Experience' }, { value: '100%', label: 'Contractor Focused' }].map((stat, i) => (
            <div key={i} style={{ animation: `countUp 0.5s ease forwards`, animationDelay: `${0.5 + i * 0.1}s`, opacity: 0 }}>
              <div style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 800, color: 'var(--accent)', lineHeight: 1 }}>{stat.value}</div>
      ...
      <Section background="white" style={{ padding: '5rem 0 2rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          {services.map((svc, idx) => (
            <FadeIn key={idx}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '4rem', alignItems: 'center', marginBottom: '7rem', direction: svc.flip ? 'rtl' : 'ltr' }}>
                <div style={{ direction: 'ltr', position: 'relative' }}>
                  <div style={{ borderRadius: '20px', overflow: 'hidden', boxShadow: '0 30px 60px rgba(11,31,51,0.15)', position: 'relative', height: '380px' }}>
                    <Image src={svc.img} alt={svc.alt} fill style={{ objectFit: 'cover' }} />
                    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'linear-gradient(135deg, rgba(217,154,43,0.15) 0%, transparent 50%)', pointerEvents: 'none' }} />
                  </div>
                  <div style={{ position: 'absolute', top: '-18px', right: '-18px', width: '56px', height: '56px', borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.6rem', boxShadow: '0 8px 24px rgba(217,154,43,0.4)', animation: 'pulseGlow 3s ease-in-out infinite', zIndex: 2 }}>
                    {svc.icon}
                  </div>
                </div>
                <div style={{ direction: 'ltr' }}>
                  ...
                  <ul style={{ listStyleType: 'none', paddingLeft: 0, marginBottom: '2rem' }}>
                    {svc.items.map((item, i) => (
                      <li key={i} style={listItemStyle}><span style={listIconStyle}>▸</span> {item}</li>
                    ))}
                  </ul>
      ...
      <Section background="dark" style={{ textAlign: 'center', padding: '7rem 0', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ maxWidth: '800px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <FadeIn>
            ...
            <p style={{ fontSize: '1.1rem', opacity: 0.8, marginBottom: '2.5rem' }}>Talk to us about your programme, delay or commercial challenge.</p>
