...
const problems = [
  { problem: "We are delayed but cannot prove why.", happens: "Delay events are discussed on site but not recorded or linked back to programme impact.", help: "Delay registers, event tracking, programme updates and critical path reviews.", icon: "🔴", number: "01" },
  { problem: "Our EOT claim needs stronger support.", happens: "The claim exists, but records, programme analysis and cause-effect narrative are weak.", help: "EOT support, delay analysis, chronology, supporting documentation and impact assessment.", icon: "📄", number: "02" },
  { problem: "Our programme is out of date.", happens: "The baseline programme no longer reflects delivery and recovery actions are unclear.", help: "Programme updates, recovery programmes, lookahead planning and critical path analysis.", icon: "📅", number: "03" },
  { problem: "We do not have a full-time planner.", happens: "PMs, CAs or site managers are trying to update programmes while delivering the job.", help: "Flexible remote planning support without permanent overhead.", icon: "👷", number: "04" },
  { problem: "We are exposed to liquidated damages.", happens: "Delays are not linked to excusable events, creating risk at the end of the project.", help: "Delay tracking, EOT support, programme evidence and contemporaneous records.", icon: "⚠️", number: "05" },
  ...
  return (
    <>
          ...
          <StaggerContainer className={styles.grid2} style={{ alignItems: 'stretch' }}>
            {problems.map((item, index) => (
              <StaggerItem key={index}>
                <div style={{ background: 'var(--white)', borderRadius: '20px', padding: '2.5rem', boxShadow: '0 4px 24px rgba(11,31,51,0.06)', border: '1px solid rgba(11,31,51,0.05)', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', top: '1rem', right: '1.5rem', fontSize: '4rem', fontWeight: 900, color: index % 2 === 0 ? 'rgba(217,154,43,0.07)' : 'rgba(11,31,51,0.05)', lineHeight: 1, userSelect: 'none' }}>{item.number}</div>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', marginBottom: '1.5rem' }}>
                    <div style={{ fontSize: '1.8rem', flexShrink: 0, animation: `floatUp ${3 + index * 0.4}s ease-in-out infinite`, animationDelay: `${index * 0.3}s` }}>{item.icon}</div>
      ...
      <Section background="dark" style={{ padding: '5rem 0', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '2rem', position: 'relative', zIndex: 1 }}>
          <FadeIn>
            <div>
