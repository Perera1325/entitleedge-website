  ...
  return (
    <>
      <PageHero
        ...
        title="Industries"
        subtitle="Supporting contractors across infrastructure and construction sectors — wherever complex programmes, delay risk and commercial exposure exist."
        extra={
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M30 5L55 20v30L30 55 5 50V20Z' fill='none' stroke='rgba(217,154,43,0.04)' stroke-width='1'/%3E%3C/svg%3E")`, backgroundSize: '60px 60px' }} />
        }
      >
        <div style={{ display: 'flex', gap: '0.75rem', marginTop: '2.5rem', flexWrap: 'wrap' }}>
          {['Roads', 'Rail', 'Utilities', 'Airports', 'Marine', 'Buildings', '+More'].map((tag, i) => (
            <span key={i} style={{ padding: '0.35rem 0.9rem', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '50px', color: 'rgba(255,255,255,0.8)', fontSize: '0.8rem', fontWeight: 500 }}>{tag}</span>
          ))}
        </div>
      </PageHero>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <StaggerContainer className={styles.grid3} style={{ alignItems: 'stretch' }}>
            {industries.map((ind, i) => (
              <StaggerItem key={i}>
                <div style={{ background: 'var(--white)', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 4px 24px rgba(11,31,51,0.07)', border: '1px solid rgba(11,31,51,0.05)', display: 'flex', flexDirection: 'column', height: '100%' }}>
                  <div style={{ position: 'relative', height: '200px', overflow: 'hidden' }}>
                    <Image src={ind.img} alt={ind.title} fill style={{ objectFit: 'cover' }} sizes="(max-width: 768px) 100vw, 33vw" />
                    ...
                    <div style={{ position: 'absolute', top: '0.75rem', right: '0.75rem', padding: '0.25rem 0.65rem', background: 'rgba(217,154,43,0.9)', borderRadius: '50px', fontSize: '0.7rem', fontWeight: 700, color: '#fff', letterSpacing: '0.5px', textTransform: 'uppercase' }}>{ind.tag}</div>
                  </div>
